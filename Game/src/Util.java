...
public class Util {
    ...
    public static void log(final String event, WorkFlow workFlow) {
        ...
        if (event.equals(Constants.KILL)) {
            logKill(workFlow);
        }
        if (event.equals(Constants.AFTER_MOVE)) {
    ...
    private static void logKill(WorkFlow workFlow) {
        if (!workFlow.turnFirstPlayer) {
            System.out.println("You killed " + workFlow.player1.name + " at " + workFlow.player1.position.x + " , " + workFlow.player1.position.y);
        } else {
            System.out.println("You killed " + workFlow.player2.name + " at " + workFlow.player2.position.x + " , " + workFlow.player2.position.y);
        }
        System.out.println("Now he should go to 0, 0");
    }

    public static WorkFlow initializeWorkFlow(String name1, String name2) {
    ...
    public static boolean initialCheck(final WorkFlow workFlow) {
        if (workFlow.turnFirstPlayer) {
            return workFlow.player1.position.x == 0 && workFlow.player1.position.y == 0;
        } else {
            return workFlow.player2.position.x == 0 && workFlow.player2.position.y == 0;
        }
    }

    public static boolean isKill(WorkFlow workFlow) {
        if((workFlow.player1.position.x==0 && workFlow.player1.position.y==0) || (workFlow.player2.position.x==0 && workFlow.player2.position.y==0))
            return false;
        return (workFlow.player1.position.x == workFlow.player2.position.x) && (workFlow.player1.position.y == workFlow.player2.position.y);
    }
}
